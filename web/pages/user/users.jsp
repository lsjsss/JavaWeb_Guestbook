		      ...
		      <tbody>
		      	<c:forEach items="${userList}" var="obj">
			        <tr>
			          ...
			          <td>${obj.roleName }</td>
			          <td>
			          	<a href="${ctx}/user.do?type=get&id=${obj.id}">编辑</a>
			          	<a href="${ctx}/user.do?type=delete&id=${obj.id}" onclick="return confirm('确实要删除编号为 【 ${obj.id}】 的记录吗?');">删除</a>
			          </td>
			        </tr>
		        </c:forEach>		        
		      </tbody>
